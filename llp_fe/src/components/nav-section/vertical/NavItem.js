...
export default function NavItem({
  ...
  const renderContent = (
    <StyledItem depth={depth} active={active} disabled={disabled}
      ...
      caption={!!caption} active2={activeSubItem.toString()} {...other}>
      {
        icon &&
      ...
      {(children?.length === 0) ? (<></>) : (
        <Iconify
          width={16}
          icon={open ? 'eva:arrow-ios-upward-outline'
  ...
  const renderItem = () => {
    ...
    // Default
    return (
      <NextLink href={PATH_DASHBOARD.root + path} passHref style={{ textDecoration: 'none' }}>
        {renderContent}
      </NextLink>
    );
  };
