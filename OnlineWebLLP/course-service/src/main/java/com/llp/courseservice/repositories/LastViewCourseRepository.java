...
public interface LastViewCourseRepository extends JpaRepository<LastViewCourse, LastViewCourseKey> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE lastViewCourse SET lastViewCourse.courseId = :courseId WHERE lastViewCourse.userId = :userId", nativeQuery = true)
    void update(@Param("courseId") String courseId, @Param("userId") int userId);

    @Query(value = "SELECT lastViewCourse.courseId FROM lastViewCourse WHERE lastViewCourse.userId = :userId", nativeQuery = true)
