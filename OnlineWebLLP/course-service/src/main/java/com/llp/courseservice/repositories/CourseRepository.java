...
public interface CourseRepository extends PagingAndSortingRepository<Course, UUID>, JpaRepository<Course, UUID> {
    ...
    @Query(value = "SELECT course.id, course.name, course.updatedAt, course.duration, level.name AS level, course.overview, course.target\n" +
            ...
            "         level ON course.levelId = level.id\n" +
            "WHERE    course.id = :id", nativeQuery = true)
    CourseOverview getOverviewById(@Param("id") String id);

    @Query(value = "SELECT course.id, course.imageLink, course.name, course.createdBy, course.rating, course.ratingNum, course.price\n" +
            "FROM     course\n" +
            "WHERE  course.isProminent = 1", nativeQuery = true)
    List<CourseCard> getAllProminentCourse(Pageable pageable);

    @Query(value = "SELECT course.id, course.imageLink, course.name, course.createdBy, course.rating, course.ratingNum, course.price\n" +
            "FROM     course INNER JOIN\n" +
            "         courseTopic ON course.id = courseTopic.courseId INNER JOIN\n" +
            "         topic ON courseTopic.topicId = topic.id\n" +
            "WHERE  topic.id = :topicId AND course.isProminent = 1\n" +
            "ORDER BY course.ratingNum DESC", nativeQuery = true)
    List<CourseCard> getAllProminentCourseByTopicId(@Param("topicId") int topicId, Pageable pageable);

    @Query(value = "SELECT course.id, course.imageLink, course.name, course.createdBy, course.rating, course.ratingNum, course.price\n" +
            "FROM     course INNER JOIN\n" +
            "         courseTopic ON course.id = courseTopic.courseId INNER JOIN\n" +
            "         topic ON courseTopic.topicId = topic.id INNER JOIN\n" +
            "         subCategory ON topic.subCategoryId = subCategory.id\n" +
            "WHERE  subCategory.id = :subCategoryId AND course.isProminent = 1\n" +
            "ORDER BY course.ratingNum DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT course.id, course.imageLink, course.name, course.createdBy, course.rating, course.ratingNum, course.price\n" +
            ...
            "         courseTopic ON course.id = courseTopic.courseId INNER JOIN\n" +
            "         topic ON courseTopic.topicId = topic.id INNER JOIN\n" +
            "         subCategory ON topic.subCategoryId = subCategory.id INNER JOIN\n" +
            "         category ON subCategory.categoryId = category.id\n" +
            "WHERE  category.id = :categoryId AND course.isProminent = 1", nativeQuery = true)
